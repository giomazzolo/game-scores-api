...
func (sq *ScoreQuery) QueryGame() *GameQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, score.GameTable, score.GameColumn),
		)
		fromU = sqlgraph.SetNeighbors(sq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (sq *ScoreQuery) WithGame(opts ...func(*GameQuery)) *ScoreQuery {
	query := (&GameClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ScoreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Score, error) {
	...
	if sq.withUser != nil || sq.withGame != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withGame; query != nil {
		if err := sq.loadGame(ctx, query, nodes, nil,
			func(n *Score, e *Game) { n.Edges.Game = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ScoreQuery) loadGame(ctx context.Context, query *GameQuery, nodes []*Score, init func(*Score), assign func(*Score, *Game)) error {
